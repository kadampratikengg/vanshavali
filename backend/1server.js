...
// Debug: Verify adminAuthRoutes
console.log('Admin Auth Routes:', adminAuthRoutes.stack.map(r => ({
  path: r.route?.path,
...
app.get('/admin/test', (req, res) => {
  res.status(200).json({ message: 'Admin route prefix is working' });
});
  ...
  try {
    ...
    app._router.stack.forEach((r) => {
      if (r.route && r.route.path) {
        ...
        if (prefix === 'admin') {
          adminAuthRoutes.stack.forEach((adminRoute) => {
            if (adminRoute.route) {
              console.log(`    Admin Route: ${Object.keys(adminRoute.route.methods).join(', ').toUpperCase()} /admin${adminRoute.route.path}`);
            }
          });
        }
      }
