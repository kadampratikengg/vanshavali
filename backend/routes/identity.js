...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    if (!identity) {
      return res.status(200).json({ personalData: {}, identityData: [] });
    }
...
router.put('/', authenticateToken, async (req, res) => {
  try {
    ...
    const updatedIdentityData = identityData.map(item => ({
...
router.post('/document', authenticateToken, async (req, res) => {
  try {
    ...
    if (!documentType || documentType === 'Select Document' || !documentNumber || !fileUrl) {
      return res.status(400).json({ message: 'Document type, number, and file URL are required' });
    }
    let identity = await Identity.findOne({ userId: new mongoose.Types.ObjectId(req.user.id) });
    if (!identity) {
      identity = new Identity({ userId: new mongoose.Types.ObjectId(req.user.id), personalData: {}, identityData: [] });
    }
    const newDocument = {
    ...
    identity.identityData.push(newDocument);
    await identity.save();
...
router.delete('/document/:id', authenticateToken, async (req, res) => {
  try {
    ...
    identity.identityData = identity.identityData.filter(doc => doc._id.toString() !== req.params.id);
    await identity.save();
